...
3. **`stats_*.csv` or JSONs missing the `Source` column.** Rebuild with `-gline-tables-only`. ATT also needs symbols to attribute to source — `strip` will silently break it.
4. **Outputs split across many files, one per CU / SE / dispatch.** `rocprofv3 --att` runs the **`rocprof-trace-decoder`** library automatically (bundled in ROCm ≥ 7.13; install the `rocprof-trace-decoder` package on older ROCm, or pass `--att-library-path <dir>`). It emits, under `att_<tag>/`: `stats_*.csv` (per-instruction latency summary), `ui_output_agent_<id>_dispatch_<id>/*.json` (the UI tree), raw `.att` SQTT binaries, and `.out` code-object copies. There is **no** `att_tool` binary — open the `ui_output_*/` tree with **`rocprof-compute-viewer`**, or `rglob("att_<tag>/**/*.json")` to aggregate the JSONs in Python.
