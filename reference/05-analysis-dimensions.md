...
PC sampling's stochastic CSV (`<pid>_pc_sampling_stochastic.csv`) has one row per sample with these columns: `Sample_Timestamp`, `Exec_Mask`, `Dispatch_Id`, `Instruction`, `Instruction_Comment`, `Correlation_Id`, `Wave_Issued_Instruction`, `Instruction_Type`, `Stall_Reason`, `Wave_Count`. The `Stall_Reason` column is populated only when `Wave_Issued_Instruction == 0`. Aggregate by `Stall_Reason` and by source line: when the binary was built with `-gline-tables-only`, rocprofv3 populates a `Source` column directly (group by that); otherwise group by the raw `Instruction` PC and resolve hot PCs to `file:line` by hand with `llvm-addr2line -e <binary>`. `extract_stall_hotspots.py` does the grouping but not the `addr2line` resolution — see [`04-python-api.md`](04-python-api.md). The `host_trap` CSV (`<pid>_pc_sampling_host_trap.csv`) lacks `Stall_Reason`; use it only for per-line hotspots, not a breakdown.
