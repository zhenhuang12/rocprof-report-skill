...
- **Per-PC ATT data is split across many JSON files** under `att_<tag>/ui_output_agent_<id>_dispatch_<id>/`: rglob `att_<tag>/**/*.json` and aggregate. (`rocprof-trace-decoder` runs automatically inside `rocprofv3 --att`; visualize the `ui_output_*/` tree with `rocprof-compute-viewer`. There is no `att_tool` binary.)
