...
3. **Run three (sometimes four) profiles** — write all outputs to `profile/<run_name>/reports/`. See [`reference/03-collection.md`](reference/03-collection.md) for the full recipes.
   ...
   - **(d) — only if you need CU timeline / tail-effect shape:** `rocprof-compute profile --timeseries-sampling-rate 1ms -p profile/<run_name>/reports/rpc_ts_<tag>`. Dimension 5 / Pattern M depend on this CSV.
