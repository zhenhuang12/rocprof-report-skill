...
3. **Run three profiles**: (a) `rocprofv3 --kernel-trace --hip-trace --hsa-trace` for the overview timeline, (b) `rocprof-compute profile -k <kernel-substring>` for the section-based perf metrics (the AMD analog of `ncu --set full`; roofline is on by default, use `--no-roof` to skip), and (c) per-line stall attribution (the AMD analog of `ncu --set source --section SourceCounters`) — **prefer PC sampling** via `rocprofv3 --pc-sampling-beta-enabled --pc-sampling-method host_trap --pc-sampling-unit time` (or `--pc-sampling-method stochastic` with `cycles`/`instructions` on MI300+ where supported); **fall back to ATT/SQTT** via `rocprofv3 --att` only when PC sampling isn't available. Write outputs to `profile/<run_name>/reports/`. If your analysis later needs **CU timeline / tail-effect shape**, also run a 4th pass: `rocprof-compute profile --timeseries-sampling-rate 1ms -p profile/<run_name>/reports/rpc_ts_<tag>` — Dimension 5 / Pattern M depend on this CSV. See [`reference/03-collection.md`](reference/03-collection.md).

4. **Parse with Python** — `pandas` for the CSVs, `sqlite3` for `.rpd` / `.db` files, and `rocpd` / `rocprof-analyze` for cross-section queries — not by eye-balling the CLI. Write analysis outputs to `profile/<run_name>/analysis/`. Use the helpers under `$SKILL_ROOT/helpers/` — invoke them with their full path (`python3 "$SKILL_ROOT/helpers/analyze_reports.py" --run-dir "$PROFILE_RUN_DIR" ...`), passing `$PROFILE_RUN_DIR` rather than `cd`-ing into it. If `$SKILL_ROOT` isn't set, derive it once: `SKILL_ROOT=$(dirname "$(realpath SKILL.md)")` (from this file's directory). See [`reference/04-python-api.md`](reference/04-python-api.md).
